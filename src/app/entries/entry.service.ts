...
export class EntryService {
  ...
  getSeeds(id: number): Observable<Seeds> {
    ...
    console.log(sUrl);
    return this.httpjson.get<Seeds>(sUrl).pipe(
  ...
  getFireEntries(poolsId): Observable<any[]> {
    return this.afs.collection('picks', ref => ref.where('poolsId', '==', poolsId)).valueChanges();
  }

  getFireEntry(poolsId: number, id: number): Observable<any> {
    //console.log('In Firestore Entry');
    return this.afs
      .collection('picks', ref => ref.where('poolsId', '==', poolsId).where('id', '==', id))
      .valueChanges();
  }
  ...
  getFireIndex(poolsId: number): Observable<any> {
    //console.log('In Firestore Entry');
    return this.afs
      .collection('numOfEntries', ref => ref.where('poolsId', '==', poolsId))
      .valueChanges();
  }

  updateFireIndex(id: string) {
    this.afs
      .collection('numOfEntries')
      .doc('y1S2XNMMwqh8vOOMrkgW')
  ...
  getEntries(): Observable<Entry[]> {
    return this.http.get<Entry[]>(this.entriesUrl).pipe(
      ...
      tap(
        data => console.log(data.map(obj => obj.entryName)),
  ...
  getEntryNames(): Observable<Entry[]> {
    return this.http.get<any[]>(this.entriesUrl).pipe(
  ...
  createEntry(entry: Entry): Observable<Entry> {
    ...
    return this.http
      .post<Entry>(this.entriesUrl, entry, { headers })
      .pipe(
